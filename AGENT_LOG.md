...
## 2026-08-29 — CONV-RECORD (`cd3524c2-3c65-4b53-8584-9f6dd7fa91b7`): the durable conversation record (feature-runner)

Implemented the durable conversation record, its encode/decode with dual-path bounds, the two-phase
durable write path, recovery/replay, and the SIGKILL crash-injection proof. NOT the HTTP route/CLI
(that is the next task, CONV-CREATE-CLI) and NOT create-idempotency (also CONV-CREATE-CLI).

- **Package/location:** `internal/store` (task proof_cmd targets `./internal/store`, test
  `TestConversationRecord`; parallel-safety note says "touches internal/store"). New files only:
  `conversation.go` (record type, `ConversationRecordKind = "conversation"`, encode/decode,
  validation, id minting), `conversationstore.go` (durable `ConversationStore`: `wal.Applier` +
  `Create` + recovery), `conversation_test.go`, `conversation_crash_test.go`. `internal/store`
  gains an `internal/wal` import (new edge; no cycle — wal does not import store).
- **Id (ruling 2):** `store.NewConversationID(busID)` mints `<busID>.<uuid-v4>`. The uuid is a
  crypto/rand-backed RFC 4122 v4, generated by setting the version/variant bits over 16 random bytes
  and rendering canonical lowercase 8-4-4-4-12 — stdlib only, NO uuid dependency (invariant 8). The
  whole id is server-authoritative (invariant 1); validated (bus half via `ids.ValidateBusID`, uuid
  half strict lowercase-canonical v4) on BOTH construct and disk-decode paths.
- **Name (ruling 3, CONV-NAME-INV6):** optional, `<= 128` bytes, valid UTF-8, single-line printable
  — rejects any C0/C1 control (U+0000-001F, U+007F-009F), U+2028, U+2029, invalid UTF-8. REFUSE, not
  truncate. Enforced on the construction path (`Encode`) AND the disk-decode path
  (`DecodeConversationRecord`) via one shared `validate()` — the dual-enforcement crux — each with a
  test (`TestConversationRecordNameBoundConstruct`, `...NameBoundDecode`).
- **Fields (invariant 6, metadata/routing only, no bodies):** server-minted id, fully-qualified
  creator (invariant 2), optional bounded name, recipient list (1..64 FQ agent ids, no duplicates),
  created-at. No `Pos` field (per `message.go` reasoning). Strict decoder (unknown fields + trailing
  data refused, re-validated). `record_version` 1.
- **Durability (invariants 4, 5):** `Create` writes through the existing two-phase `wal.Log.Write`
  (Begin→fsync PREPARE, Commit→fsync COMMIT), then folds the CANONICAL record in. `ConversationStore`
  is a `wal.Applier` built-before-Open / Attach-after (same three-step as ack/invite/relay). Cap
  `MaxConversations` (65536) fail-closed on live Create only; replay never refuses a durable record.
- **Crash-injection proof (CLAUDE.md):** `conversation_crash_test.go` — a real SIGKILL child at
  three write-path points on a second Create. commit-fsynced ⇒ BOTH conversations recovered
  (invariant 4); prepare-fsynced ⇒ only the first, second discarded, burned index never re-handed
  (invariants 5 and 1, proven by a post-recovery Create's PrepareIndex > burned); before-write ⇒
  only the first, no dangling. `Apply` discards a tampered committed record loudly at ERROR
  (invariant 6) and never errors.
- **Reservation:** NONE taken. `wal-entry-kind` = 3 = `"conversation"` already existed (2026-08-15);
  business records ride the existing TypePrepare/TypeCommit frames, so no `record-type` /
  `ondisk-format-version` reservation. `tags` from the task description were OMITTED: CONV-NAME-INV6
  ruled only on the NAME; there is no ruling admitting a tag set, and adding an un-ruled metadata
  field would risk invariant 6 — a tag set is CONV-CREATE-CLI/a future ruling's concern.
- **Invariants read IN FULL:** 1 (server-authoritative ids, never reused, sequence advances past a
  discarded index), 2 (fully-qualified `<bus-id>.<agent-id>`), 4 (durable before ack), 5 (recover to
  a prefix), 6 (metadata/routing only, loud discard, keyed MAC — reused, not added), 8 (stdlib uuid,
  no new dep).
- **Verify:** `go build ./...`, `go vet ./...` clean; `"$(go env GOROOT)/bin/gofmt" -l` on the four
  files EMPTY. Full `go test -race ./...` GREEN (all packages, working-tree @ f6411146). Clean-overlay
  of HEAD + the four owned files: `bash scripts/proof-check.sh "go test -race -run 'TestConversationRecord'
  ./internal/store"` → `verdict=PASS class=test tests_run=44 top_level=11 skipped=0 failed=0`;
  crash tests green in the overlay too.
- **Gates:** reviewer — COMPLETED (see CONV-RECORD journal). security — COMPLETED/REQUIRED (durable
  record + invariant-6 name-bound is a log-injection surface; no docs-and-tests-only carve-out
  applies, product code changed). Documentation: CONTRACTS-ONDISK.md updated here (new
  `wal-entry-kind` namespace table + the `"conversation"` record section); AGENT_PROTOCOL.md /
  CONTRACTS-CLI held for CONV-CREATE-CLI (no agent-facing route yet).
...
## 2026-08-28 — RELAY-23 + RELAY-53: relay envelope wire version + two-resolver coexistence

**Tasks:** RELAY-23 (`220d36f4-e420-4dfc-9f82-b2af1b8653e6`), RELAY-53
(`d5bbdec9-0659-423e-a420-183133ab9818`) — co-landed.

**Change (production, `internal/relay`):**
- `message.go`: added `RelayRequest.ProtocolVersion int json:"protocol_version,omitempty"` (first
  field); `WireVersion = 1` (spends reserved `relay-wire-version = 1`), `relayWireVersionAbsent = 0`,
  `ErrUnsupportedRelayVersion`, `resolveWireVersion`; `RelayedMessage.ProtocolVersion` (resolved
  provenance); `ValidateRelayRequest` check 0 resolves the version before any field it governs
  (refuses unknown, never defaults); `Forward` stamps `WireVersion` on egress. MaxRelayBytes
  derivation comment updated (+25 bytes, still ~2.5x headroom under 256 KiB).
- `handshake.go`: `CodeUnsupportedRelayVersion = "unsupported_relay_version"`.
- `peer.go`: `ErrorCode` maps `ErrUnsupportedRelayVersion` → `CodeUnsupportedRelayVersion`, above the
  SIGN-7/`ErrInvalidRelay` arms so it is not shadowed by a vaguer code.
- `client.go`: `peerErrorCode` allow-list carries `CodeUnsupportedRelayVersion` (partial-rollout
  legibility, mirroring the ACK code).
- `wireversion_relay23_test.go` (new): round-trip of `protocol_version:1` and the distinct-key
  assertion (RELAY-23 proof `TestRelayEnvelopeCarriesDistinctWireVersionKey`); resolver behaviour,
  two-resolver independent coexistence, ingest refusal (validate + HTTP 400, no disconnect), absent
  reads as 1, egress stamping (RELAY-53 proof regex `TestWireVersion`).

**RELAY-53 decision:** the two resolvers COEXIST (distinct names/sentinels/codes), NOT collapsed. See
DECISIONS.md 2026-08-28. No compile collision — ACK-3 named `resolveAckWireVersion` defensively.

**Invariants read in full:** 1 (version reserved not chosen; server authoritative on egress version),
2 (FQ ids, unchanged), 6 (envelope = metadata/framing only; version is outside the signature like
bus_path), 10 (unrecognised version REFUSED not defaulted; reject-and-log, 400 not 503, no disconnect).

**Verification:** `go build ./...` green, `go vet ./internal/relay/` clean, gofmt empty output.
RED-before evidence (feature-runner): mutation A (default unknown → 1) fails the resolver + ingest
proofs; mutation B (JSON key → "version") fails the distinct-key proof; both restored green. Full
`go test -race ./...` and mutation coverage handed to test-engineer; reviewer + security gates run
before completion (verdicts in the task journal).

**Gates:** reviewer — REQUIRED (new product code). security — REQUIRED (touches
CONTROL-PLANE-adjacent wire versioning / downgrade surface; not docs-and-tests-only). documentation —
