...
| Not crash-safe yet | A pending onward hop is **not** re-offered after a restart. The durable outbox record survives, but the intermediate does not retain the origin's attestation, so `Resume` cannot rebuild the envelope and settles the job as abandoned (logged at WARN). `RELAY-48`. Locally-originated hops are unaffected and DO resume. |
