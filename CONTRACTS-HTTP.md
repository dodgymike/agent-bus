...
| Onward relay | **Still not implemented.** A message that arrives here FROM a peer is delivered to this bus's own agents only and is never carried to a further hop (`relay.AcceptOptions.Onward` is nil, and the egress adapter forwards only messages originated here). "Forwards to a peer" and "relays onward" are different claims; only the first is true. |
