...
- **Do NOT run the whole correctness suite.** Your brief carries a full `go test -race ./...` result
  — its command, the sha it ran against, and pass/fail — that the orchestrator ran ONCE for the
  panel. Trust it for correctness and spend your runs on MEASUREMENT instead: `go test -bench`,
  `pprof`, a timed replay of a large log, load against the running binary. Do not re-run `./...` for
  correctness. If the result is absent, HEAD moved since it ran, or you have a concrete reason to
  distrust one result, say so and run the SPECIFIC test — still not `./...`. If the shared result
  cites `working-tree @ <sha>`, treat it as ADVISORY and measure against the LIVE files regardless —
  a sha match cannot detect further uncommitted edits.
