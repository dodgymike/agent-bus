...
- **Do NOT run the whole test suite.** Your brief carries a full `go test -race ./...` result — its
  ...
  Trust it and run only the narrow checks specific to YOUR dimension (the crash-injection test for
  the touched invariant, a single `-run` recovery test, killing the process mid-write and
  restarting), never `./...`. Every panelist re-running the suite is wasted duplicate work. If the
  ...
  `working-tree @ <sha>`, treat it as ADVISORY and run your dimension's checks against the LIVE files
  regardless — a sha match cannot detect further uncommitted edits.
