...
- [~] None · DISCOVERY-DOC: self-describing unauthenticated discovery document so an agent with only a bus URL can bootstrap — httpapi, P1, in progress
  ...
  _Proof: go test -race ./internal/httpapi/... && curl -s http://127.0.0.1:PORT/v1/discovery | jq ._
...
- [ ] DISCOVERY-DOC-FU-CLI · DISCOVERY-DOC-FU-CLI: `agent-busctl` subcommand that fetches and renders the bus discovery document (+ AGENT_PROTOCOL.md / CONTRACTS-CLI.md entries) — cli, P2
...
- [~] None · Whole-log quarantine reissued EVERY sequence number ever minted -- fixed by a durable index floor outside the log (invariant 1, second instance) — durability, P0, in progress
...
- [~] None · CLI-1-FU-BINARYNAME: Decide the INSTALLED name of the client binary — cli, P1, in progress
...
- [~] INVITE-STORE · INVITE-STORE: durable single-use invite record (mint/lookup/consume/expire), recovered by WAL replay, with a crash-injection test — auth, P0, in progress
...
- [ ] None · MTLS-PIN-FU-MIRRORSYNC: an agreement test that client.BusFingerprint and internal/buscert.Fingerprint cannot silently diverge — security, P2
...
- [~] MTLS-PIN · MTLS-PIN: the client PINS the bus's certificate fingerprint and hard-fails on a change -- never InsecureSkipVerify, and no flag that silently disables verification — security, P0, in progress
...
- [~] MTLS-BUSCERT · MTLS-BUSCERT: generate/load the bus's self-signed certificate + private key in the data dir (0600), fatal if unusable — security, P0, in progress
