...
- [ ] None · AUTH-1-FU-ACTIVECAP: cap ACTIVE sessions per agent -- the one place an agent-id-keyed cap is SAFE — auth, P1
...
- [~] None · AUTH-1-FU-LISTENADDR: default listen address is :8080 (all interfaces) but DECISIONS.md settled on localhost — auth, P0, in progress
  ...
  _Proof: grep -n "defaultListen" cmd/agent-bus/main.go | grep -q "127.0.0.1" && grep -qE "^\| \`-listen\` \| \`127\.0\.0\.1:8080\` \|" CONTRACTS.md && echo ALL_OK_
