...
- [ ] None · No transport security (TLS) anywhere in the server -- BLOCKED ON USER DECISION — security, P1, blocked
...
- [~] None · CONTRACTS-SPLIT: split CONTRACTS.md into per-plane files (pure move) + retarget every proof_cmd — documentation, P1, in progress
...
- [ ] None · DUR-11-FU-CONTRACTS: CONTRACTS.md still documents the reverted refuse-to-start WAL policy verbatim — documentation, P1
  Discovered during the DUR-11 orphaned-task reconciliation pass (2026-08-02). CONTRACTS.md:277-282 states "The only damage wal.Open repairs automatically is a provably torn tail ... Anything else -- a bad magic, a wrong format version, a commit naming no open prepare, a payload that will not decode -- refuses to start and leaves the file byte-for-byte unchanged." That is the OLD refuse-to-start policy; under the shipped always-restart policy (DUR-11, commits 0c122fa/6bb9f6c) three of those four cases now START and DISCARD. CONTRACTS.md:234 also still says "a torn-tail repair truncates bytes" (function is now RepairLog, not RepairTail). CONTRACTS.md has ZERO mention of RepairLog, the quarantine rename-aside behaviour, the bus.wal.corrupt-<ns> / bus.wal.repair on-disk artefacts, or the Repair/Recovered struct fields (Rewritten, Quarantined, DiscardCount, MissingRecords, Exhausted) -- all of which reviewer flagged as a mandated per-change deliverable (CLAUDE.md step 9) that was never done. Fix: rewrite CONTRACTS.md WAL-repair section to match the shipped RepairLog/quarantine/always-restart behaviour, including the new on-disk artefact names and the Repair/Recovered fields actually surfaced to callers.
  _Proof: grep -n "provably torn tail\|refuses to start and leaves the file byte-for-byte\|RepairTail" CONTRACTS-ONDISK.md; test $? -ne 0_
...
- [ ] None · LISTENADDR-FU-CONTRACTS: CONTRACTS.md CLI-flag table still shows -listen default :8080 — docs, P1
...
- [ ] None · Whole-log quarantine reissues EVERY sequence number ever minted -- startup must refuse, not resume from 1 (invariant 1, second instance) — durability, P0
  ...
  *** CONSENT-SENSITIVE -- CONFIRM WITH THE USER BEFORE IMPLEMENTATION ***. The user reverted a broader refuse-to-start policy once already. The fail-closed DIRECTION follows from invariant 1 as ruled, but the specific mechanism (what marks 'legitimately empty' vs 'quarantined-unknown', where that marker is durably recorded, and the exact refuse-to-start condition) needs explicit sign-off before code is written, not just inferred from the earlier ruling on the tail-salvage case.
  ...
  _Proof: bash scripts/proof-check.sh 'go test -race -run TestRecover_WholeLogQuarantine_RefusesStartOnUnprovenSequenceFloor ./internal/wal'_
...
- [~] None · AUTH-1-FU-ACTIVECAP: cap ACTIVE sessions per agent -- the one place an agent-id-keyed cap is SAFE — auth, P0, in progress
...
- [~] None · AUTH-1-FU-LISTENADDR: default listen address is :8080 (all interfaces) but DECISIONS.md settled on localhost — auth, P0, in progress
...
- [x] DUR-1 · DUR-1: WAL record framing + writer — durability, P0
  ...
  _Proof: go test -race ./internal/wal/_
- [~] DUR-12 · DUR-12: Replace CRC32C with an HMAC-SHA256 keyed MAC (ON-DISK FORMAT CHANGE, reserved ondisk-format-version=2) -- UNBLOCKED, key lives in data dir mode 0600 — durability, P0, in progress
...
- [ ] None · Invariant-1 narrowing (quarantined-tail index reissue) is documented only in a code comment -- needs DECISIONS.md + CONTRACTS.md, flag for USER DECISION — durability, P1
  internal/wal/recover.go (around lines 44-67) now documents that a damaged TAIL record has its index REISSUED on the next successful write -- "ids that were OBSERVED are never reissued for as long as the records containing them survive... the crash case (a torn final frame whose Append never returned) reissues an index" -- which means a client-observed id CAN be handed out twice across this recovery path. CLAUDE.md invariant 1 states ids are never reused, including across restarts, with no carve-out. This is presented as an accepted consequence of the 2026-08-02 availability-over-retention decision, but an invariant NARROWING recorded only in a source comment is not actually recorded per this project's own rules (DECISIONS.md is the append-only record of design decisions; CONTRACTS.md documents every record type/behavior). Two things needed: (1) a DECISIONS.md entry making the invariant-1 narrowing explicit -- under what exact conditions an index can be reissued, and why that is judged acceptable; (2) a CONTRACTS.md note on the WAL record-recovery behavior reflecting the same. FLAG FOR USER DECISION: this needs explicit user sign-off if the reuse window is observable by a client that already received an ack for the original (pre-crash) use of that index -- i.e. does an at-least-once-delivery client ever see the SAME index claimed by two different pieces of content, and if so is that tolerable. Do not resolve this narrowing unilaterally; surface the question in the DECISIONS.md entry for the user to answer.
  _Proof: grep -qi 'reissue' DECISIONS.md && grep -qi 'reissue' CONTRACTS.md_
...
- [~] ID-2-WIRING-SEAL · ID-2-WIRING-SEAL: Sequence refuses to issue from an UNSEALED floor (the only half implementable today) — ids, P0, in progress
