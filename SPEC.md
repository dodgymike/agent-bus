...
- [ ] None · DUR-11-FU-CONTRACTS: CONTRACTS.md still documents the reverted refuse-to-start WAL policy verbatim — documentation, P1
  ...
  _Proof: grep -n "provably torn tail\|refuses to start and leaves the file byte-for-byte\|RepairTail" CONTRACTS.md; test $? -ne 0_
...
- [~] AUTH-2 · AUTH-2: Token verification middleware — auth, P0, in progress
  ...
  _Proof: go test -race -run 'TestAuthMiddleware|TestEveryRouteRequiresAuth' ./internal/httpapi_
...
- [~] None · AUTH-1-FU-PENDINGCAP: MaxPendingPerAgent is a lockout primitive, not a defence -- rekey or drop it — auth, P0, in progress
...
- [~] None · AUTH-1-FU-LISTENADDR: default listen address is :8080 (all interfaces) but DECISIONS.md settled on localhost — auth, P0, in progress
  ...
  _Proof: grep -n 'defaultListen' cmd/agent-bus/main.go | grep -q '127.0.0.1' && echo LISTEN_OK; grep -n '8080' README.md CONTRACTS.md | grep -qi localhost && echo DOCS_OK_
...
- [~] None · Startup summary silently omits whole-log quarantine (quarantined/discard_count/discarded_bytes never logged) — durability, P0, in progress
