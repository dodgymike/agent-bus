...
- [~] None · busctl send/broadcast lose the minted idempotency key on an ambiguous failure — cli, P1, in progress
...
- [~] None · busctl watch help documents a fatal 503 under exit 5 but client.ExitCode yields 6 — agentif, P1, in progress
...
- [ ] CLI-5-FU-WIDTH · CLI-5-FU-WIDTH: busctl agents should use the real terminal width, not a fixed 100 columns — cli, P3
  ...
  _Proof: go test -race -run 'TestCLIAgentsHumanTableNeverTruncatesQualifiedID|TestCLIAgentsRespectsColumns' ./cmd/busctl/..._
- [ ] CLI-3-FU-SAFETEXT · CLI-3-FU-SAFETEXT: export the terminal-safe renderer from client/ and delete busctl's copy — cli, P2
  ...
  _Proof: go test -race -run 'TestCLIWatchHumanFeedNeutralisesTerminalEscapes|TestSafeText' ./client/... ./cmd/busctl/... && ! grep -n 'func terminalSafe' cmd/busctl/watch.go_
...
- [~] IDEM-11 · IDEM-11: Durable applied-key store, recovered via WAL replay, with a bounded retention window — core, P0, in progress
  ...
  _Proof: bash scripts/proof-check.sh 'go test -race -run TestIdemCrash ./internal/hub/'_
...
- [~] RELAY-2 · RELAY-2: Message relay + ongoing roster sync across peers — relay, P2, in progress
...
- [~] RELAY-3 · RELAY-3: Loop prevention via traversed-bus path — relay, P2, in progress
