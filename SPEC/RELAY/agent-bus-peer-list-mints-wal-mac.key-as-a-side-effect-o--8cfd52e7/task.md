# agent-bus peer list mints wal-mac.key as a side effect of a read-only command

| Field | Value |
| --- | --- |
| Public id | `8cfd52e7-6cbd-4d29-b34d-c3dee87e73e5` |
...
| Section | backlog |
| Tags | — |
| Created | 2026-08-21T14:14:52.166429+00:00 |
| Updated | 2026-08-21T14:15:22.949705+00:00 |
| Completed | — |

## Proof command

```sh
go test -race -run 'TestPeerListMACKeyGuard|TestPeerListMACKeyFixtureMintsControl' ./cmd/agent-bus
```

## Description

FILED 2026-08-21 by spec-keeper, from RELAY-54's security gate. SIBLING TASK: the identical defect
at `agent-bus operator list` -- `b5089ddf-5a5a-41e0-8278-036f6a195e2a` (`agent-bus operator list`).

# The defect

`openPeerStore`'s READ-ONLY path (`writable=false`) calls `wal.Replay` at
`cmd/agent-bus/peer.go:1459` with NO MAC-key guard.
...
`agent-bus peer list -data-dir <dir>`:

    exit=1
    before: bus-id bus.wal
    after : bus-id bus.lock bus.wal wal-mac.key      <-- 65-byte wal-mac.key MINTED
    agent-bus peer list: replaying the write-ahead log: wal: <dir>/bus.wal: corrupt at offset 0:
...
`cmd/agent-bus/peer.go` ALREADY has exactly the right guard shape, for the OTHER thing an offline reader must
never mint: ``checkPeerBusIDPresent` (`peer.go:1405` pre-lock, `:1431` post-lock)` is called PRE-lock and RE-checked UNDER the lock, which is what makes
`ids.LoadOrCreateBusID`'s "Create" half unreachable. Verified at HEAD 1cc881f: pointed at a
directory with a 3-byte `bus.wal` and NO `bus-id`, ``agent-bus peer list`` exits 4 and the directory gains
...
`log` and `outbox` both use exit **5** for "unverifiable". `cmd/agent-bus/peer.go` ALREADY uses 5 for
``exitPeerUnknown` (`peer.go:208`) -- "`peer remove` withdrew nothing"`, so reusing it would give one code two documented meanings and break a caller that
scripts against it. Allocate the next free code (`exitPeerUnverifiable = 6`) in that file's block, document it in
...
Sibling: `b5089ddf-5a5a-41e0-8278-036f6a195e2a` -- agent-bus operator list, the same defect at the other call site. `relates` edges recorded both ways.
...
- **relates to** [RELAY-54](../RELAY-54--911841af/task.md)
