...
func TestIdemCrashChild(t *testing.T) {
	...
	case idemCrashLegacyEntry:
		...
		m, err := store.NewMessage(testBusID, sender, false, []string{recipient}, 1, time.Now().UTC(), idemCrashBody, idemCrashKey)
		if err != nil {
...
func TestIdemCrashPostCommitRetryReplaysTheOriginal(t *testing.T) {
	...
	if len(committed) != 1 {
		t.Fatalf("the crashed log holds %d committed entries, want exactly 1: the child died before its send was durable, so there is no post-commit crash to recover from", len(committed))
	}
	...
	if after := replayCommitted(t, dir); len(after) != 1 {
		t.Fatalf("after the retry the durable log holds %d committed entries, want 1: the retry wrote a SECOND effect to disk", len(after))
	}
	if got := h.IdempotencyStats().Count; got != 1 {
...
func TestIdemCrashDanglingPrepareLeavesTheKeyUnseen(t *testing.T) {
	...
	// (0) Nothing committed — the crash really did land between the two fsyncs.
	if got := replayCommitted(t, dir); len(got) != 0 {
		t.Fatalf("the crashed log holds %d committed entries, want 0: the child was supposed to die with its prepare unresolved", len(got))
	}
	...
	if rec.Applied != 0 {
		t.Fatalf("Recovered().Applied = %d, want 0: an uncommitted prepare must not be applied", rec.Applied)
	}
	...
	if got := replayCommitted(t, dir); len(got) != 1 {
		t.Fatalf("the durable log holds %d committed entries after the first attempt, want exactly 1", len(got))
	}
	...
	retry, err := h.Send(hub.SendRequest{
		Sender:         sender,
		To:             recipient,
		Body:           idemCrashBody,
		IdempotencyKey: idemCrashKey,
